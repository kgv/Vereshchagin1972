...
В адсорбционной хроматографии поток жидкости вызывает селективную миграцию компонентов смеси в пористой среде с высокой адсорбирующей способностью из узкой стартовой зоны. В большинстве случаев адсорбентом для разделения липидов служит силикагель или кремнекислота. Эти термины обозначают соединения, близкие по составу. Они представляют собой сыпучие тела с микропористой структурой, соответствующие формуле #ce("SiO2 · H2O") и образующиеся из щелочного силиката в процессе дегидратации и полимеризации ортокремневой кислоты #ce("H4SiO4") в сильнокислой среде. Силикагель можно получить из #ce("H4SiO4") и в солевом растворе, например в растворе #ce("NН4Cl"). Такой силикагель не содержит на поверхности участков с кислыми свойствами и как адсорбент существенно отличается от силикагеля, полученного в кислой среде [339]. Из отечественных сортов силикагеля для адсорбционной хроматографии чаще всего применяется силикагель КСК [1а, 6].

Внутренняя часть зерен кремнекислоты и силикагеля представляет собой силоксан тетраэдрического строения (#ce("—Si—O—Si—")), лишенный адсорбционных свойств. Адсорбирующая способность материала связана исключительно с поверхностными группами силанола (#ce("—Si—OH")), которые обычно удерживают при помощи водородных связей мономолекулярный слой «свободной» воды [785]:

#skeletize(config: alchemist_config, {
  single()
  fragment("Si")
  branch({
    single(angle: 6)
...
// operator(math.stretch(sym.arrow.l.l, size: 2em))
// operator($<-$, margin: 1em)
// single()
// fragment("HC")
// single()
...
//   fragment("CH")
//   branch({
//     single(absolute: 0deg)
//   })
//   single()
//   fragment("O")
//   single()
// })
// fragment("CH", vertical: true)
// branch({
//   single(angle: 1)
//   fragment(name: "O1", "O")
// })
// single()
// fragment("HC", vertical: true)
// single()
// fragment("HC", links: (
//   "O1": single(from: 1),
// ))
...
#skeletize(
  ...
  ),
  {
    ...
    operator(math.stretch(sym.lt))

    single(atom-sep: 0.5)
    fragment("HC")
    cycle(3, absolute: 0deg, {
      single()
      fragment("CH")
      branch({
        single(absolute: 0deg, atom-sep: 0.5)
      })
      single()
      fragment("O")
      single()
...
Окись магния применяли для анализа липидов лишь в начальный период развития адсорбционной хроматографии, в связи с тем что этот адсорбент малоэффективен. Так, для отделения олеиновой кислоты от стеариновой на колонке #ce("MgO") требуется более 17 суток. Аддукты жирных кислот и 2,4-динитробензосульфохлорида разделяли на #ce("MgSO4") [114, 305, 877]. В настоящее время #ce("MgO") и #ce("MgSO4") в хроматографии липидов не используются.

Метилолеат, -линолеат и -линоленат, и также их цис-, транс-изомеры разделяли на катионообменных смолах, пропитанных #ce("AgNO3") [263, 1002].
...
// Условия обнаружения насыщенных и ненасыщенных нейтральных липидов и жирных кислот приведены в табл. 2. Лучшим неспецифическим проявителем для всех липидов является Родамин 6 G в 2 н. растворе NaOН; после опрыскивания желтые пятна липидов на пурпурном фоне наблюдают и фотографируют в УФ-свете [427]. Абсолютная и относительная величины Rƒ при адсорбционной хроматографии сильно зависят от концентрации отдельных компонентов, свойств бумаги и других условий разделения. Индивидуальные классы или фракции липидов элюируют с бумаги метанольным раствором HCI или другими растворителями и подвергают гидролизу или метанолизу [583, 932].
