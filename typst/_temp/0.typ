...
#align(center)[
  #text(1.7em, weight: "bold")[Биохимия триглицеридов]
...
#strong[Биохимия триглицеридов.] А. Г. Верещагин. М., «Наука», 1972 г.

Монография посвящена вопросам современной биохимии липидов. Она включает следующие основные разделы: методы качественного и количественного анализа триглицеридов и жирных кислот (адбсорбционная, распределительная и газо-жидкостная хроматография), кристаллизация и противоточное распределение триглицеридов, современные теории строения триглицеридов, состав триглицеридов важнейших животных и растительных жиров, биосинтез триглицеридов.
...
#align(center)[
  ...
  #strong[Л. Д. БЕРГЕЛЬСОН]
]

#heading(level: 1, numbering: none)[Сокращения]

#strong[Сокращенные обозначения жирных кислот и их ацилов в триглицеридах:]

#grid(
  columns: (1fr, 1fr),
  gutter: 1cm,
  [Az — азелаиновая], [О — олеиновая],
  ...
  [М — миристиновая], [Э — эландиновая],
  [Ma — масляная], [],
)

#strong[Другие обозначения:]

/ A, B, C: любые жирные кислоты и их ацилы в составе глицеридов; [A]$sub(1)$, [А]$sub("1-3")$, [А]$sub(2)$ — содержание А в 1-, 1,3- и 2-положениях глицеридов в мол %
/ ДГ: диглицериды
/ e: число двойных связей липида
/ $Delta$e: разность между числом двойных связей в двух разных липидах
/ ФК: фосфатидные кислоты
/ m: число атомов углерода в алифатических цепях липида
/ $Delta$m: разность между числом атомов углерода двух разных липидов или разных ацилов одного липида
/ МГ: моноглицериды
...
/ [S]$sub(2)$: содержание насыщенных кислот в 2-положении глицеридов в мол %
/ S$sub(3)$, SSS: тринасыщенные триглицериды
/ [S$sub(3)$]$sub(RD)$: содержание S$sub(3)$ в триглицеридах, вычисленное по теории статистического распределения
/ SSU: 1,2(2,3)-динасыщенно-3(1)-ненасыщенные триглицериды в ПТС

#pagebreak()

/ S$sub(2)$U: динасыщенно-мононенасыщенные триглицериды в типовом составе
/ SUU: 1(3)-насыщенно-2,3(1,2)-диненасыщенные; триглицериды в ПТС
/ SU$sub(2)$: мононасыщенно-диненасыщенные триглицериды в типовом составе
/ SUS: 1,3-динасыщенно-2-ненасыщенные триглицериды в ПТС
/ СЖК: свободные жирные кислоты
/ ТГ: триглицериды
/ U: ненасыщенные жирные кислоты и их ацилы в составе глицеридов
/ [U]$sub(2)$: содержание ненасыщенных ацилов в 2-положении глицеридов в мол %
/ U$sub(3)$, UUU: триненасыщенные триглицериды
/ USU: 2-насыщенно-1,3-диненасыщенные триглицериды в ПТС
/ V$sub(R)$: удерживаемый объем в распределительной и газо-жидкостной хроматографии
/ [X]: экспериментально найденное содержание вещества Х в мол %
/ 18 : 2$super(9,12)$: неразветвленная жирная кислота с 18 атомами углерода и двумя цис-двойными связями, находящимися в положениях 9 и 12

#heading(level: 1, numbering: none)[Триглицериды природных жиров (введение)]

#heading(level: 2, numbering: none)[ПРИРОДНЫЕ ЖИРЫ И МАСЛА КАК СМЕСИ ТРИГЛИЦЕРИДОВ ЖИРНЫХ КИСЛОТ]
...
#block(stroke: (left: 1pt + gray))[
  #grid(
    columns: (auto, auto),
    gutter: 2cm,
    [
      CH$sub(2)$-O-CO-R$sub(1)$ \
      $|$ \
      R$sub(2)$-CO-O-CH \
      $|$ \
      CH$sub(2)$-O-CO-R$sub(3)$ \
      Триглицериды
    ],
    [
      CH$sub(2)$-O-CH$sub(2)$-CH$sub(2)$-R$sub(1)$ \
      $|$ \
      R$sub(2)$-CO-O-CH \
      $|$ \
      CH$sub(2)$-O-CO-R$sub(3)$ \
...
#heading(level: 2, numbering: none)[ИСТОРИЯ ИССЛЕДОВАНИЯ, НОМЕНКЛАТУРА И ИЗОМЕРИЯ ТРИГЛИЦЕРИДОВ]
...
#heading(level: 3, numbering: none)[Жирнокислотный состав]

#strong[Групповые категории состава]

+ Углеродный состав — величины содержания групп жирных кислот, характеризуемых тем или иным числом атомов углерода _(m)_, в % от суммы кислот.
+ Олефиновый состав — величины содержания групп жирных кислот, характеризуемых тем или иным числом двойных связей _(e)_, в % от суммы кислот.
+ Фракционный состав — величины содержания фракций жирных кислот, характеризуемых тем или иным значением полярности молекулы (PN), в % от суммы кислот.

#strong[Индивидуальные категории состава]

+ Типовой состав — величины содержания суммы насыщенных кислот ([S] = [$super(1)$S] + [$super(2)$S] + ... + [$super(n)$S]) и суммы ненасыщенных кислот ([U] = [$super(1)$U] + [$super(2)$U] + ... + [$super(n)$U]), в % от суммы кислот (_n_ — число жирных кислот в жире).
+ Видовой состав — величины содержания индивидуальных видов жирных кислот ([$super(1)$S], [$super(2)$S], ..., [$super(n)$S]; [$super(1)$U], [$super(2)$U], ..., [$super(n)$U]), характеризуемых тем или иным числом атомов углерода и двойных связей, в % от суммы кислот.
+ S-видовой состав — величины содержания суммы насыщенных кислот ([S]), а также индивидуальных ненасыщенных кислот ([$super(1)$U], [$super(2)$U], ..., [$super(n)$U]), характеризуемых тем или иным числом атомов углерода и двойных связей, в % от суммы кислот.
+ Изомерно-видовой состав — величины содержания изомерных видов жирных кислот, характеризуемых тем или иным числом атомов углерода и двойных связей, а также тем или иным положением двойных связей в цепи, в % от суммы кислот.
+ Оксикислотный состав — величины содержания индивидуальных жирных кислот, характеризуемых — тем или иным числом атомов углерода и двойных связей, а также числом гидроксильных групп, в % от суммы кислот.
+ Возможные полиморфные формы видов жирных кислот данного жира.

#heading(level: 3, numbering: none)[Триглицеридный состав]

#strong[Групповые категории состава]

+ Углеродный состав — величины содержания групп триглицеридов, характеризуемых тем или иным числом атомов углерода суммы ацилов _(m)_, в % от суммы триглицеридов.
+ Олефиновый состав — величины содержания групп триглицеридов, определяемых тем или иным числом двойных связей _(e)_, в % от суммы триглицеридов.
+ Фракционный состав — величины содержания фракций триглицеридов, характеризуемых тем или иным значением полярности молекулы (PN), в % от суммы триглицеридов.
+ Углеродно-фракционный состав — величины содержания групп триглицеридов, определяемых тем или иным числом атомов углерода, в каждой из фракций триглицеридов, характеризуемых тем или иным значением полярности молекулы, в % от суммы триглицеридов.

#strong[Индивидуальные категории состава]

+ Типовой состав — величины содержания типов триглицеридов, характеризуемых тем или иным типовым составом жирных кислот ([S$sub(3)$], [S$sub(2)$U], [SU$sub(2)$], [U$sub(3)$]), в % от суммы триглицеридов.
+ Позиционно-типовой состав — величины содержания позиционных типов триглицеридов, характеризуемых тем или иным типовым составом жирных кислот, а также центральным или краевым положением ацилов отдельных типов в глицеридной молекуле ([SSS], [SSU], [SUS], [SUU], [USU], [UUU]), в % от суммы триглицеридов.

#pagebreak()

+ Видовой состав — величины содержания индивидуальных видов триглицеридов, характеризуемых тем или иным видовым составом жирных кислот (например, [О$sub(2)$Л], [ПО$sub(2)$] и др.; П — остаток пальмитиновой кислоты), в % от суммы триглицеридов.
+ Позиционно-видовой состав — величины содержания позиционных видов триглицеридов, характеризуемых тем или иным видовым составом жирных кислот, а также центральным или краевым положением ацилов отдельных видов в молекуле глицерида (например, [ОЛО], [ООЛ] и др.), в % от суммы триглицеридов.
+ S-Видовой состав — величины содержания индивидуальных триглицеридов, характеризуемых тем или иным S-видовым составом жирных кислот (например, [SO$sub(2)$], [S$sub(2)$Л] и др.), в % от суммы триглицеридов.
+ Позиционно-S-видовой состав — величины содержания индивидуальных триглицеридов, характеризуемых тем или иным S-видовым составом жирных кислот, а также центральным или краевым положением насыщенных ацилов и отдельных видов ненасыщенных ацилов в молекуле глицерида (например, [SOO], [S$sub(2)$IS] и др.), в % от суммы триглицеридов.
+ U-Видовой состав — величины содержания индивидуальных триглицеридов, характеризуемых тем или иным видовым составом насыщенных кислот, а также типовым составом ненасыщенных кислот (например, [ПU$sub(2)$], [С$sub(2)$U] и др.), в % от суммы триглицеридов.
+ Позиционно-U-видовой состав — величины содержания индивидуальных триглицеридов, характеризуемых тем или иным видовым составом насыщенных кислот, а также типовым составом ненасыщенных кислот и центральным или краевым положением отдельных видов или типов ацилов в молекуле глицерида (например, [ПUU], [CUC] и др.), в % от суммы триглицеридов.
+ Стереовидовой состав — величины содержания индивидуальных триглицеридов, характеризуемых тем или иным видовым составом жирных кислот, а также 1-, 2- или 3-положением отдельных видов ацилов (например, _sn_-1-пальмито-2,3-диолеин и др.), в % от суммы триглицеридов.
+ Возможные полиморфные формы триглицеридов стереовидового состава данного жира.
...
#align(center)[
  #strong[Таблица 1] \
  #strong[Комбинированные методы определения отдельных категорий жирнокислотного и триглицеридного состава природных жиров]
]
...
#table(
  ...
  [], [], [*А*], [*Б*], [*В*], [*Г*], [*Д*], [*Е*], [*Ж*], [*И*], [*К*],
  [*I*], [10а], [1], [], [], [], [], [], [], [], [],
  [*II*], [11им \ 14а 14б], [2 \ 2р \ 4и], [2], [2], [15б], [], [14у \ 18аим], [10ж \ 16бдеж], [18абв], [],
  [*III*], [23м], [9], [9], [], [], [14], [], [23ж], [], [],
  [*IV*], [12би \ 16аб], [3], [3], [2], [17бе], [14 \ 18аи], [], [16аби], [], [],
  [*V*], [12бд \ 13д \ 17б-д], [3], [3], [5], [17абв], [12], [15бг], [], [], [],
  [*VI*], [11абв], [8], [8], [2], [17аб \ 19абв \ 22абвг], [14х \ 14б \ 15бг], [14ан], [11бф], [11 16бд \ 17абгд], [],
  [*VII*], [10б 16б \ 17бвг], [1], [1], [], [17абв], [10 \ 17вгк \ 20бк \ 21бгк], [16бвк], [10ж], [], [],
  [*VIII*], [], [5 \ 7с], [5], [], [17е], [17вгм], [], [], [], [],
  [*IX*], [], [6л], [6л], [], [19л], [], [], [], [], [],
  [*X*], [23жп], [], [], [], [], [], [], [], [], [],
  [*XI*], [22бе \ 23п], [8 \ 9], [8 \ 9], [], [22абде], [23], [], [], [], [],
  [*XII*], [23ж], [], [], [], [], [], [], [23ж], [], [],
)

#strong[Примечание.] Арабские цифры — номера категорий жирнокислотного и триглицеридного состава (стр. 12), определяемые данным сочетанием аналитических методов.

#strong[Количественные физические методы разделения и анализа:] I — перегонка; II — кристаллизация; III — определение температуры плавления; IV — жидкостно-жидкостная экстракция или противоточное распределение; V — обращенно-фазовая распределительная хроматография: VI — тонкослойная или адсорбционная хроматография; VII — газо-жидкостная хроматография на неполярной фазе; VIII — газо-жидкостная хроматография на полярной фазе; IX — УФ-спектроскопия; Х — ИК- и ЯМР-спектроскопия; XI — определение оптического вращения; XII — рентгеноструктурный анализ.

#strong[Количественные химические методы анализа:] XIII — щелочной гидролиз; XIV — то же, что и XIII, и метилирование возникших при гидролизе жирных кислот; XV — то же, что и XIV, и образование комплексов эфиров с ионами Ag$super(+)$; XVI — липазный гидролиз; XVII — переэтерификация; XVIII — окислительное расщепление по двойным связям; XIX — присоединение к двойным связям водорода, галоидов или озона; XX — образование комплексов с ионами Ag$super(+)$.

#strong[Продукты химических превращений исходных триглицеридов (А):] Б — жирные кислоты; В — метиловые эфиры жирных кислот; Г — метиловые эфиры насыщенных кислот и комплексы метиловых эфиров ненасыщенных кислот с ионами Ag$super(+)$; Д — смесь исходных триглицеридов, жирных кислот, 1,2(2,3)-диглицеридов и 2-моноглицеридов; Е — статистическая смесь триглицеридов; Ж — смесь S$sub(3)$, S$sub(2)$A$sub(z)$, SA$sub(z2)$, и A$sub(z3)$ (A$sub(z)$ — остаток азелаиновой кислоты); И — тринасыщенные триглицериды, галоидопроизводные триглицеридов или озониды триглицеридов соответственно; К — тринасыщенные триглицериды и комплексы ненасыщенных триглицеридов с ионами Ag$super(+)$.
